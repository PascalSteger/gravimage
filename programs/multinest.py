...
def run(LogLikelihood,
    ...
    dumper_type  = CFUNCTYPE(c_void_p,            # nSamlpes
                             c_int,               # nlive
                             c_int,               # nPar
                             c_int,               # physLive
    ...
    print('before __nested_MOD_nestrun, in multinest.py')
...
# @param nest_ef defines the sampling efficiency. 0.8 and 0.3 are recommended
#    for parameter estimation & evidence evalutation
#    respectively.
#    use 'parameter' or 'model' to select the respective default
#    values
# @param nest_Ztol MultiNest can find multiple modes & also specify which samples belong to which mode. It might be
...
# @param nest_tol A value of 0.5 should give good enough accuracy.
# @param nest_nCdims If mmodal is T, MultiNest will attempt to separate out the
...
#@param null_log_evidence If mmodal is T, MultiNest can find multiple modes & also specify
...
# @param nest_outfile write output files? This is required for analysis.
# @param dump_callback a callback function for dumping the current status
