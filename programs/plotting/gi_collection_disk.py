...
#        unit = '[Msum/kpc^3]'
        ...
        if len(self.profs)>0:
            if prof == 'chi2':
                ...
                bins, edges = np.histogram(np.log10(goodchi), range=[-2,6], \
                                           bins=max(6,np.sqrt(len(goodchi))), density=True)
    ...
    def plot_bins(self, ax, prof, gp):
        ...
        return
    ## \fn plot_bins(self, ax, prof, gp)
    # plot vertical line at the positions of the bin centers
    # @param ax axis object
    # @param prof string to identify profile which is currently plotted
    # @param gp global parameters
